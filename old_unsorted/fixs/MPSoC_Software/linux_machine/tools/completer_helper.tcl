...
#      processorConfigure - uses variable activePSperipheralsList to enable selected peripherals
#      VIOadd
#      LEDsLinearAdd
#      buttonsRosettaAdd
#      LEDsRosettaAdd
...
#    2016.3    - WK - 01/13/2017 - addition of new procs, further testing of existing procs, includsion of UED similar capabilities
#    initial   - WK - 11/10/2016 - based on many other completer scripts
#
# ***********************************************************************
#
...
proc projectCreate {} {
   ...
   set isUBlaze          [strsame $platform "MicroBlaze"]
   ...
   if {[logicValue $isLangNotSelected]} {
      ...
      puts "Please type: use ZC702 | Zed | ZCU102 | RFSoC (where supported)";    # todo: add specific MPSoC and RFSoC boards
      puts "   then rerun the projectCreate"
   } else {
     # future - verify that "latestVersion" will work for the boards
     if {[logicValue $isZed]} {
         create_project -force $labName C:/training/$tcName/$demoOrLab -part xc7z020clg484-1
         set_property board_part em.avnet.com:zed:part0:1.3 [current_project]
     } elseif {[logicValue $isZC702]} {
         create_project -force $labName C:/training/$tcName/$demoOrLab -part xc7z020clg484-1
         set_property board_part xilinx.com:zc702:part0:1.2 [current_project] 
     } elseif {[logicValue $isZCU102]} {
         create_project -force $labName C:/training/$tcName/$demoOrLab -part xczu9eg-ffvb1156-2-i
         set_property board_part xilinx.com:zcu102:part0:3.0 [current_project]       
     } elseif {[logicValue $isRFSoC]} {
         create_project -force $labName C:/training/$tcName/$demoOrLab -part xczu25dr-ffve1156-1-i
...
proc sourcesAdd { sourceList } {
   ...
   foreach fileName $sourceList { 
     ...
     if {[llength $hierarchyList] > 1} {        # a hierarchy has been presented so use it instead of the support directory 
        ...
        set fullFileName ""
        append fullFileName c:/training/$tcName/support/ $fileName
        ...
        if {$isVHDLsource == 0 && $isVerilogSource == 0} {    
           if {$isVHDL == 0} { append fullFileName .vhd }
          if {$isVerilog == 0} { append fullFileName .v }
        }
        ...
        set useThisFile C:/training/$tcName/support/$fileName.vhd
      }
   ...
   foreach fileName $sourceList { 
     ...
     if {[llength $hierarchyList] > 1} {        # a hierarchy has been presented so use it instead of the support directory 
        ...
        append fullFileName c:/training/$tcName/support/ $fileName
         ...
         set isVHDLsource    [strEndsWith $fileName .vhd]
         set isVerilogSource [strEndsWith $fileName .v]
        if {!($isVHDLsource) && !($isVerilogSource)} {     
        ...
        #add_files -norecurse $fullFileName
        # this line copies the file to the local working directory keeping the original file unchanged     
        set useThisFile C:/training/$tcName/support/$fileName
      }
...
proc constraintFilesDefaultAdd {} {
   ...
   if {$isZC702} {
     #add_files -fileset constrs_1 -norecurse C:/training/$tcName/support/ZC702_base.xdc
     add_files -fileset constrs_1 -norecurse C:/training/CustEdIP/ZC702_base.xdc
   } elseif {$isZed} {
     #add_files -fileset constrs_1 -norecurse C:/training/$tcName/support/Zed_base.xdc
     add_files -fileset constrs_1 -norecurse C:/training/CustEdIP/Zed_base.xdc
   }
   ...
   if {$isUBlaze} {
     if {$isZC702} {
       set clkConstraintFile C:/training/$tcName/support/ZC702_sys_clk.xdc
       if {[fileExists $clkConstraintFile]} {
         add_files -fileset constrs_1 -norecurse $clkConstraintFile
       }
     } elseif {$isZed == 0} {
       set clkConstraintFile C:/training/$tcName/support/zed_sys_clk.xdc
       if {[fileExists $clkConstraintFile]} {
         add_files -fileset constrs_1 -norecurse $clkConstraintFile
       }
     }
   ...
   if {[llength $constraintFileList] > 0 } {      
      foreach fileName $constraintFileList {   
       ...
       # is there a full path provided? - Does not make corrections for langauage - assumes user knows what he/she's doing
        ...
        add_files -fileset constrs_1 -norecurse $fullFileName
      }   
   } else {
...
proc applyZynqProcessorPreset {} {
   ...
   if {$isUBlaze == 0} {
      # If using ublaze, no need to do anything wity presets.
   } else {
      if {$isZC702 == 0} {
...
proc processorAdd {} {
   ...
   if {$isUBlaze} {
     ...
     # since the uB is (by definition) running in an APSoC or MPSoC device, we have to use the special IP to access the rx, tx, and hp0 ports of the PS
     ...
     if {![containedIn xilinx.com:user:XparentPS:1.0 $availableIPs]} {
        set_property  ip_repo_paths  C:/training/CustEdIP [current_project]
        update_ip_catalog    
     }
     create_bd_cell -type ip -vlnv [latestVersion xilinx.com:user:XparentPS:1.0] PS_access
...
proc processorConfigure {} {
   ...
   if {$isUBlaze} {
     ...
     if {$verbose} { puts "configuring PS." }
     if {$isZC702} {
...
proc processorDefaultConnect {} {
   ...
   variable ZynqAllOff
   variable MPSoCallOff
   variable activePSperipheralsList
...
proc VIOadd {} {
   ...
   set isUBlaze [strsame $processor microblaze_0]
   ...
   # connect the clock
   if {$isUBlaze} { connect_bd_net [get_bd_pins vio_fmcce/clk] [get_bd_pins clk_wiz_1/clk_out1]; } 
...
proc blockDesignWrap {} {
   ...
   set fullPath ""
   append fullPath c:/training/$tcName/$demoOrLab/$labName.srcs/sources_1/bd/$blockDesignName/$blockDesignName.bd
   make_wrapper -files [get_files $fullPath] -top
   
   set fullPath ""
   if {[string compare -nocase [getLanguage] verilog] == 0} {
      append fullPath C:/training/$tcName/$demoOrLab/$tcName.srcs/sources_1/bd/$blockDesignName/hdl/$blockDesignName\_wrapper.v    
   } else {
      append fullPath C:/training/$tcName/$demoOrLab/$labName.srcs/sources_1/bd/$blockDesignName/hdl/$blockDesignName\_wrapper.vhd  
   }
...
proc hardwareManagerOpen {} {
   ...
   connect_hw_server;   #
   open_hw_target;      #
   set_property PROGRAM.FILE {C:/training/$tcName/lab/$tcName.runs/impl_1/$blockDesignName.bit} [get_hw_devices xc7z020_1]
   set_property PROBES.FILE {C:/training/$tcName/lab/$tcName.runs/impl_1/$blockDesignName.ltx} [get_hw_devices xc7z020_1]
   set_property FULL_PROBES.FILE {C:/training/$tcName/lab/$tcName.runs/impl_1/$blockDesignName.ltx} [get_hw_devices xc7z020_1]
...
proc designExport {} {
   ...
   file mkdir C:/training/$tcName/lab/$tcName.sdk
   
   # copy the system definition over to the SDK workspace
   set sysDefPath ""
   append sysDefPath C:/training/$tcName/lab/$tcName.runs/impl_1/$blockDesignName _wrapper.sysdef
   set hdfPath ""
   append hdfPath C:/training/$tcName/lab/$tcName.sdk/$blockDesignName _wrapper.hdf
   file copy -force $sysDefPath $hdfPath 

   markLastStep designExport
   
#file copy -force C:/training/HWSW_Xdebug/lab/HWSW_Xdebug.runs/impl_1/embd_dsgn_wrapper.sysdef C:/training/HWSW_Xdebug/lab/HWSW_Xdebug.sdk/embd_dsgn_wrapper.hdf
#launch_sdk -workspace C:/training/HWSW_Xdebug/lab/HWSW_Xdebug.sdk -hwspec C:/training/HWSW_Xdebug/lab/HWSW_Xdebug.sdk/embd_dsgn_wrapper.hdf
}
#
# ***** launch SDK
#
proc SDKlaunch {} {
   ...
   set hdfPath ""
   append hdfPath C:/training/$tcName/lab/$tcName.sdk/$blockDesignName _wrapper.hdf
   launch_sdk -workspace C:/training/$tcName/lab/$tcName.sdk -hwspec $hdfPath
...
proc make {stopAfterStep} {
   ...
   if {[strcmp $stopAfterStep "all"] == 0} {
      set stopAtStepN [llength $stepList]
      puts "All detected. Changed to $stopAfterStep"
   } else {
